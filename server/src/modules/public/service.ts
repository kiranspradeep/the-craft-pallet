...
export const publicService = {
  ...
  getProductBySlug: async (slugOrId: string) => {
    const product = await publicRepository.findProductBySlug(slugOrId);
  ...
  trackOrder: async (orderNumber: string, phone: string) => {
    ...
    const normalize = (p: string) => p.replace(/^\+?91/, "").trim();

    if (normalize(order.customer.phone) !== normalize(phone)) {
      throw new NotFoundError("Order not found");
    }
